...
public class CandyCrash {
	 ...
	 * 
	 * For each traversal of the matrix, we only check two directions, rightward and
	 * downward. There is no need to check upward and leftward because they would
	 * have been checked from previous cells.
	 * 
	 * For each cell, if there are at least three candies of the same type rightward
	 * or downward, set them all to its negative value to mark them.
	 * 
	 * After each traversal, we need to remove all those negative values by setting
	 * them to 0, then let the rest drop down to their correct position. A easy way
	 * is to iterate through each column, for each column, move positive values to
	 * the bottom then set the rest to 0.
	 */
		...
		while (found) {
			...
			for (int i = 0; i < N; i++) {
				for (int j = 0; j < M; j++) {
					...
					if (val == 0)
						continue;
					// 3 or more connected candies in a row.
					...
					// 3 or more connected candies in a column.
